...
| `/r/:roomId` | `InvestorDataRoomView` (or generic public viewer) | Public / Access-Gated | The destination for investors receiving a data room link. May require password matching or specific email verification. |
...
| `/dashboard` | `DashboardLayout`, `DashboardOverview` | Auth | Central hub. User reviews high-level stats, quick actions, and recent engagement. |
...
| `/dashboard/investors` | `InvestorExplorerPage`, Filters | Auth | Exploring the investor database to build a target list. |
| `/dashboard/outreach` | `OutreachTrackerPage`, Kanban/List | Auth | Tracking conversations, emails sent, and pitch statuses. |
