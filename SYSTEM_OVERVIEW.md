...
4. **Data Room (`/data-room`, `/r/:roomId`)**: Secure, shared spaces for hosting pitch collateral. `r/:roomId` serves as the public-facing URL shared with investors, supporting password/access tracking.
5. **Investor Explorer & Outreach (`/investors`, `/outreach`)**: CRM-like interfaces allowing founders to discover relevant investors, manage outreach pipelines, and track engagement. Integrates with `/api/outreach/send` (via Resend) to dispatch trackable pitch packages.
