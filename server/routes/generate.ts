...
router.post('/ai-template-generator', requireAuth, generalLimiter, checkAiLimits as any, async (req, res) => {
  try {
    const { prompt, userId, submitAsCommunity } = req.body;

    const systemPrompt = `You are a professional business document architect.
The user wants to generate a structure and guidance for a document matching the following prompt:
"${prompt}"

Return ONLY a valid JSON object (no markdown, no backticks, strictly parseable by JSON.parse) matching exactly this schema:
{
  "name": "string (the name of the template)",
  "document_type": "string (snake_case, e.g., pitch_deck, one_pager, financial_model, memo, business_plan)",
  "category": "string (e.g., saas, fintech, deeptech, healthtech)",
  "sector": "string (e.g., Enterprise Software)",
  "sector_tags": ["string"],
  "stage": "string (e.g., Pre-seed, Seed, Series A)",
  "stage_tags": ["string"],
  "complexity": "string (one of: light, standard, advanced)",
  "is_premium": boolean,
  "sections": [
    {
      "id": "string (snake_case unique section id)",
      "heading": "string (the section title)",
      "body": "string (Detailed instructions, prompt guidance, or boilerplate text for the user/AI to use when filling this section. Use plain text or light markdown.)"
    }
  ]
}

Ensure the sections comprehensively cover the user's request.`;
    ...
    // Generic fallback if parsing or LLM fails
    res.status(200).json({
      ...
      complexity: "standard",
      sections: [
        { id: "overview", heading: "Overview", body: "Please summarize the main purpose of this document." }
      ]
    });
  }
});

// Full Template Fill Endpoint (deprecated / filled by other mechanisms but keeping existing code)
