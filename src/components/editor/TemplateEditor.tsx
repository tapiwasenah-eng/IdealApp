...
import { ChevronDown, ChevronRight, Sparkles, Save, Download, FileText, CheckCircle2 } from 'lucide-react';
...
function SectionPreviewCard({ section, fieldValues, onFieldClick }: SectionPreviewCardProps) {
  /**
  ...
  return (
    ...
    >
      {/* Section heading */}
      {section.heading && (
        <h3 className="text-lg font-semibold leading-snug" style={{ color: section.textColor || '#111827' }}>
      ...
      {/* Subheading */}
      {section.subheading && (
        <p className="text-base" style={{ color: section.textColor ? `${section.textColor}CC` : '#4B5563' }}>
      ...
      {/* Body */}
      {section.body && (
        <p className="text-sm whitespace-pre-line leading-relaxed" style={{ color: section.textColor ? `${section.textColor}E6` : '#374151' }}>
        ...
        <ul className="space-y-1">
          {section.bullets.map((bullet, idx) => (
            <li key={idx} className="flex items-start gap-2 text-sm">
              <span className="mt-0.5 shrink-0" style={{ color: section.textColor || '#9CA3AF' }}>•</span>
              <span style={{ color: section.textColor ? `${section.textColor}E6` : '#374151' }}>
      ...
      {section.metrics && section.metrics.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
          {section.metrics.map((metric, idx) => (
            <div key={idx} className="rounded-lg bg-gray-50 border border-gray-100 px-3 py-2">
              <p className="text-xs text-gray-500 truncate">{renderHighlightedText(metric.label)}</p>
          ...
          <table className="w-full text-xs text-left">
            <thead className="bg-gray-50">
              <tr>
                {section.tableData.headers.map((header, idx) => (
                  <th key={idx} className="px-3 py-2 font-semibold text-gray-600 border-b border-gray-200">
            ...
            <tbody>
              {section.tableData.rows.map((row, rowIdx) => (
                <tr key={rowIdx} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
...
function FieldForm({ fields, sections, fieldValues, onFieldChange, focusedField }: FieldFormProps) {
                      ...
                      >
                        ...
                        {field.fieldType === 'textarea' ? (
                          <textarea
                            id={field.fieldName}
                            ref={(el) => {
                              if (el) fieldRefs.current.set(field.fieldName, el);
                            }}
                            rows={3}
...
export function TemplateEditor({
  ...
  const [localFields, setLocalFields] = useState<Record<string, string>>(initialFields || {});
  ...
  const fields = useMemo(() => extractFields(template.sections), [template.sections]);
  ...
  const handleExportNow = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${template.title.replace(/\s+/g, '-').toLowerCase()}.pdf`;
      a.click();
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            Generate Document
            {!canGenerate && (
              <span className="text-xs opacity-70">(fill {30 - progress.percentage}% more)</span>
            )}
          </button>
        </div>
      </div>
