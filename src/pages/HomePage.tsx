...
import { ConsultantChatFlow } from '../components/home/ConsultantChatFlow';
...
function HeroSection({ 
  ...
  return (
    <section className="w-full overflow-x-hidden max-w-7xl mx-auto px-6 pt-32 pb-20 min-h-[75vh] flex flex-col items-center justify-center">
      ...
      >
        ...
        <div id="ai-prompt-bar" className="w-full flex justify-center px-4">
          <div className="w-full max-w-3xl flex flex-col md:flex-row bg-white rounded-3xl md:rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100 p-2 md:items-center relative">
            ...
            <input 
              ...
              className="order-1 md:order-2 flex-1 min-w-0 bg-transparent px-4 pt-4 pb-2 md:py-3 outline-none text-slate-700 placeholder-slate-400 text-sm md:text-base resize-none"
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  const target = e.target as HTMLInputElement;
                  if (target.value.trim()) {
                    window.location.href = `/create?prompt=${encodeURIComponent(target.value)}`;
                  } else {
            ...
            <div className="order-2 md:order-1 flex items-center justify-between w-full md:w-auto p-2 md:p-0 pr-1 md:pr-3 shrink-0">
              <div className="flex items-center space-x-2">
                ...
                <div className="relative">
                  <button 
                    ...
                    className="px-3 py-1.5 h-10 shrink-0 rounded-xl border border-slate-200 bg-white shadow-sm flex items-center gap-1.5 hover:bg-slate-50 transition-colors"
                  >
              ...
              <div className="flex items-center space-x-1 md:hidden">
                ...
                <button 
                  onClick={() => {
                    const input = document.querySelector('input[placeholder*="Search your workspace"]') as HTMLInputElement;
                    if (input && input.value.trim()) {
                      window.location.href = `/create?prompt=${encodeURIComponent(input.value)}`;
                    } else {
                      window.location.href = '/create';
                    }
                  }}
...
function VideoStepsSection() {
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      } else if (currentTime >= STEPS[STEPS.length - 1].end) {
        // If we are past the last step, stay on last step or reset based on loop
          ...
          <div className="space-y-6">
            {STEPS.map((step, i) => (
              <motion.div
                ...
                className={`relative p-6 rounded-2xl transition-all duration-500 border cursor-pointer group ${
                  activeStep === i 
                    ? 'bg-violet-50 border-violet-200 shadow-lg shadow-violet-100 scale-[1.02]' 
              ...
              >
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg flex-shrink-0 transition-all duration-500 ${
                    activeStep === i ? 'bg-violet-600 text-white scale-110' : 'bg-gray-100 text-gray-400'
                  }`}>
                    {i + 1}
                  </div>
                  <div className="flex-1">
                    <h4 className={`text-xl font-black transition-colors ${
                      activeStep === i ? 'text-violet-900' : 'text-gray-400'
                    }`}>
                      {step.title}
                    </h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {TEMPLATES.map((t, i) => (
              <motion.div
                ...
                custom={i * 0.05}
                variants={fadeUp}
                className="bg-white rounded-xl border border-[#E5E7EB] p-5 hover:shadow-lg transition-all hover:-translate-y-1 cursor-pointer"
              ...
              >
                ...
                <div className="flex items-center justify-between mt-4">
                  <span className="text-sm text-[#6B7280]">{t.count}</span>
...
export default function HomePage() {
  ...
  const { addMessage, setIsGenerating } = useChatStore();

  const handlePromptSubmit = (promptObj: any) => {
    navigate('/create', { state: { prefill: promptObj } });
  };
